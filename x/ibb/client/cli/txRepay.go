...
func CmdCreateRepay() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create-repay [blockHeight] [asset] [amount] [denom]",
		Short: "Creates a new repay",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgCreateRepay(clientCtx.GetFromAddress().String(), int32(argsBlockHeight), string(argsAsset), int32(argsAmount), string(argsDenom))
...
func CmdUpdateRepay() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-repay [id] [blockHeight] [asset] [amount] [denom]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgUpdateRepay(clientCtx.GetFromAddress().String(), id, int32(argsBlockHeight), string(argsAsset), int32(argsAmount), string(argsDenom))
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
...
func CmdDeleteRepay() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "delete-repay [id] [blockHeight] [asset] [amount] [denom]",
