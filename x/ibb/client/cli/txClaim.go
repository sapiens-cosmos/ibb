...
func CmdCreateClaim() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create-claim [blockHeight] [asset] [amount] [denom]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgCreateClaim(clientCtx.GetFromAddress().String(), int32(argsBlockHeight), string(argsAsset), int32(argsAmount), string(argsDenom))
...
func CmdUpdateClaim() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-claim [id] [blockHeight] [asset] [amount] [denom]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgUpdateClaim(clientCtx.GetFromAddress().String(), id, int32(argsBlockHeight), string(argsAsset), int32(argsAmount), string(argsDenom))
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
...
func CmdDeleteClaim() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "delete-claim [id] [blockHeight] [asset] [amount] [denom]",
