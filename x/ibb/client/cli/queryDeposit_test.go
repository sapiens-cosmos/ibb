...
func networkWithDepositObjects(t *testing.T, n int) (*network.Network, []*types.Deposit) {
	...
	for i := 0; i < n; i++ {
		state.DepositList = append(state.DepositList, &types.Deposit{Creator: "ANY", Id: uint64(i)})
	}
...
func TestListDeposit(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Deposit[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Deposit)
	})
}
